...
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'media', 'text-files')
UPLOAD_KEY = os.path.join(BASE_DIR, 'media', 'public-keys')
...
def download_public_key(username):
	for root,dirs,files in os.walk(os.path.join(BASE_DIR, 'media', 'public-keys')):
		for file in files:
			file_parts = file.split('-')
			if file_parts[0] == username:
...
def download_file(filename):
	...
	if(os.path.isfile(filepath)):
		return send_file(filepath, download_name='fileMessage-SafeSendSecurity.txt',as_attachment=True)
	else:
